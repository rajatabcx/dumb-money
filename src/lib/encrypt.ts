...
function getKey(): Buffer {
  const key = process.env.INVOICE_JWT_SECRET;
  if (!key) {
    throw new Error("INVOICE_JWT_SECRET environment variable is not set.");
  }
  if (Buffer.from(key, "hex").length !== 32) {
    throw new Error(
      "INVOICE_JWT_SECRET must be a 64-character hex string (32 bytes)."
    );
  }
